...
export class CustomThrottlerGuard extends ThrottlerGuard {
  ...
  protected async throwThrottlingException(context: ExecutionContext): Promise<void> {
    ...
    response.setHeader('X-RateLimit-Limit', process.env.RATE_LIMIT_LIMIT || '10');
    response.setHeader('X-RateLimit-Window', process.env.RATE_LIMIT_TTL || '60');
    response.setHeader('Retry-After', process.env.RATE_LIMIT_TTL || '60');
    
    // Log básico da tentativa de rate limit
    console.warn(`Rate limit exceeded - IP: ${this.getClientIp(request)}, Endpoint: ${request.method} ${request.path}`);
